...
class AR4(object):
    ...
    def __init__(self, port, cal_path):
        try:
            self.calibration = {}
            self.WC = ''
            self.loop_mode = ''
            ...
            self.tool2wrist = np.eye(4,4)
            self.wrist2tool = np.eye(4,4)
            print("Init Start")
    ...
    def load_calibration(self):
        try:
            ...
            calibration = pickle.load(open(self.cal_path,"rb"))
            print("Loaded Calibration Started")
        except Exception as e:
        ...
        print("self.calibration loading complete")
    ...
    def request_T(self):
        ...
        response = str(self.ser.readline().strip(), 'utf-8')
        self.parse_response(response)
        return xyz_zyx_to_se3(float(self.calibration['XcurPos'])*0.001, float(self.calibration['YcurPos'])*0.001, float(self.calibration['ZcurPos'])*0.001, 
                          float(self.calibration['RzcurPos']), float(self.calibration['RycurPos']), float(self.calibration['RxcurPos']))
    

    def request_tool_T(self):
        wrist2world = self.request_T()
    ...
    def move_j(self, x, y, z, rz, ry, rx, j7=0.0, j8=0.0, j9=0.0,
    ...
    def move_j_ar(self, x, y, z, rz, ry, rx, j7=0.0, j8=0.0, j9=0.0,
               spd_prefix='Sp', speed=25, acceleration=20, deceleration=20, acc_ramp=100, wrist_config='F'):
    ...
    def set_tool_offsets(self, x, y, z, rz, ry, rx):
        self.tool2wrist = xyz_zyx_to_se3(x,y,z,rz,ry,rx)

        self.wrist2tool = np.linalg.inv(self.tool2wrist)
